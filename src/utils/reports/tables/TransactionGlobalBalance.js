...
const createTable = ({admin, transaction}, dates) => {
  ...
  return !!transaction?.length
    ? `
        ...
        <tr class="">
          <th class="tg-9gfu center" colspan="2"></th>
          <th class="tg-9gfu center">Depot</th>
          <th class="tg-9gfu center">Retrait</th>
    ...
    <body>
      <div id="header">
        <div class="title">
          ...
          <h3>
            BALANCE GLOBALE DES TRANSACTIONS AUX
      ...
      <table class="tg">
        ${data && data.map((data) => createTable(data, dates)).join('')}
         ${
           data &&
           createRowFooter(mergreTransaction(data), dates, transCategory)
         } 
      </table>
    </body>
  </html>
