...
const createTable = ({admin, data, mise}, dates, admins, transCategory) => {
  ...
  return `
    ...
    <tr class="label">
      ${
        transCategory === 'C'
          ? '<th class="tg-9gfu center" colspan="4"></th>'
          : '<th class="tg-9gfu center" colspan="3"></th>'
      }
    
     ${transCategory === 'C' ? '' : '<th class="tg-9gfu center">Dépot</th>'}
    ...
    ${
     ...
     ${
       transCategory === 'E'
         ? createRowFooter({mise}, `Sous-Total/Générale ${admin?.nom}`)
         : createRowFooterCredit({mise}, `Sous-Total/Générale ${admin?.nom}`)
     }
  `;
};
    ...
    <body>
      <div id="header">
        <div class="title">
          ...
          <h3>
            LISTE GLOABALE DE MEMBRES ACTIFS AUX
      ...
      <table class="tg">
        ...
        ${
          data && transCategory === 'E'
            ? createRowFooter({mise: mergreAllMise(data)}, 'Total/Générale')
            : createRowFooterCredit(
                {mise: mergreAllMise(data)},
                'Total/Générale',
              )
        }
      </table>
